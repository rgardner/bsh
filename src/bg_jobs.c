...
static struct BGJob *job_init(const pid_t pid, const struct ParseInfo *info,
                              const struct Command *cmd) {
...
static void free_job(struct BGJob *job) {
  free_info(job->info);
  free(job);
}

void put_job_in_background(const struct ParseInfo *info, pid_t pid) {
